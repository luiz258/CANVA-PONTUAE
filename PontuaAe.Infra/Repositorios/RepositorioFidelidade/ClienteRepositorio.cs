...
namespace PontuaAe.Infra.Repositorios.RepositorioAvaliacao
{
    public class ClienteRepositorio : IClienteRepositorio
    {
        ...
        {
            return await _db.Connection.QueryAsync<ListaRankingClientesConsulta>("SELECT pc.ID, c.NomeCompleto, pc.Contato, p.Segmentacao, p.SegCustomizado,  max(r.DataVenda) AS UltimaVisita, AVG(r.valor) AS GastoMedio, SUM(r.valor) AS GastoTotal, COUNT(r.DataVenda) AS QtdVisita FROM PONTUACAO  p JOIN RECEITA AS r ON p.IdEmpresa = r.IdEmpresa, PRE_CADASTRO AS pc FULL OUTER JOIN CLIENTE AS c ON pc.Contato = c.Contato WHERE p.IdEmpresa = @IdEmpresa AND p.ID = r.IdPontuacao   AND  p.IdPreCadastro = pc.ID GROUP BY pc.ID, c.NomeCompleto, pc.Contato, r.IdPontuacao, r.IdPontuacao, p.Segmentacao, p.SegCustomizado ", new { @IdEmpresa = IdEmpresa });
        ...
        {
            return await _db.Connection.QueryFirstOrDefaultAsync<IEnumerable<Cliente>>("SELECT DataNascimento, Contato FROM CLIENTE WHERE IdEmpresa = @IdEmpresa AND month(GETDATE()", new { @IdEmpresa = IdEmpresa });
        }
        ...
        {
            return await _db.Connection
              .QueryFirstOrDefaultAsync<ObterUsuarioCliente>("SELECT c.Contato, u.ID FROM USUARIO u INNER JOIN CLIENTE c ON u.ID = c.IdUsuario WHERE c.IdUsuario = @IdUsuario", new { @IdUsuario = IdUsuario });
        }
        ...
        {
            return await _db.Connection
               .QueryFirstOrDefaultAsync<ObterDadosDoCliente>(" SELECT c.NomeCompleto, pc.Contato, c.Email, c.DataNascimeto, p.Saldo,  p.Segmentacao, p.SegCustomizado, max(r.DataVenda) AS UltimaVisita, AVG(r.valor) AS GastoMedio, SUM(r.valor) AS Captado " +
               ...
               "GROUP BY c.NomeCompleto, pc.Contato, c.Email, c.DataNascimeto, p.Saldo,  p.Segmentacao, p.SegCustomizado", new {  @IdEmpresa = IdEmpresa, @IdUsuario = IdUsuario});
